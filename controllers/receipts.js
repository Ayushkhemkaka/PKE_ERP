    ...
    try {
        const rows = await query(
            `SELECT id, orderType, customerAccountName, bookNumber, slipNumber, source, date, name, site, item,
                    measurementUnit, quantity, rate, amount, freight, totalAmount, paymentStatus, dueAmount, is_printed, createdDate, orderStatus
             FROM entry
...
const markReceiptPrinted = async (req, res) => {
    ...
    try {
        ...
        const [entryRows] = await client.execute(
            `SELECT id, orderType, is_printed
             FROM entry
             WHERE id = ?`,
            [id]
        );
        ...
        await client.execute(
            `UPDATE entry
             SET is_printed = 1, lastUpdatedBy = ?
             WHERE id = ?`,
            [printedBy, id]
        );
